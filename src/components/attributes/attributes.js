...
class Attributes extends Component {
	constructor(props) {
		...
		this.state = {
			showAddAttr: false,
			message: Message.NO_ATTRIBUTE_MSG,
			searchCriteria: '',
			bannerflag: false
		};
	...
	filterAttribute = () => {
		...
		const filteredAttributes = this.props.attributes.filter(
			(att) => isContains(att.name, searchCriteria) || isContains(att.type, searchCriteria)
		);
	...
	render() {
		...
		const buttonProps = { primaryLabel: 'Add Fields', secondaryLabel: 'Cancel' };

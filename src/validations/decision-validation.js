...
export function rulesetValidation(ruleset) {
	const { expressions } = ruleset;
	const error = { expression: '', yields: '', note: '' };
	const validCase = expressions.length > 0;
	if (!validCase) {
		error.formError = 'Please specify  expressions';
	}

	// else if (yields.length === 0) {
	// 	error.formError = 'Please specify yields';
	// } else if (note === '') {
	// 	error.formError = 'Please specify note';
	// }
	return error;
}
